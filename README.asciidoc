...
:fn-x-us: footnote:x-us[Declared to be an alias of the corresponding standard library function if present.  For example, if the C library defines getpid (), then this header file will declare _getpid () to be an alias of getpid ().]
...
|       W | ={fn-x-us} | ``_getcwd (``*__buffer__``,`` __size__``);`` |
...
|       X | ={fn-x-us} | ``_vsscanf (``*__s__``,`` *__fmt__``,`` __ap__``);`` |
|         |        | |
|         |     2+a| **``▗▚▚▚▚ <libi86/stdlib.h> ▞▞▞▞▖``**
