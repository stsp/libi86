...
|       W |    G   | ``_setbkcolor (``__color__``);`` | Only partially implemented — for cases where _color_ is a compile-time constant, and is an indexed colour.
...
|     C99 |   (=)  | ``vsscanf (``*__s__``,`` *__fmt__``,`` __ap__``);`` | (C99 places this function in ``<stdio.h>``.)
