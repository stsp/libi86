...
4+|
2+|              2+| **``▗▚▚▚▚ <dir.h> ▞▞▞▞▖``**
|       B | = link:doc/implem-notes.asciidoc#user-content-dir-h[(❧)] | ``searchpath (``__file__``);`` |
|       X | = link:doc/implem-notes.asciidoc#user-content-dir-h[(❧)] | ``_searchpath (``__file__``);`` |
4+|
2+|              2+| **``▗▚▚▚▚ <direct.h> ▞▞▞▞▖``**
...
|       W |    =   | ``_getdrive ();`` |
|     P01 | (=) link:doc/implem-notes.asciidoc#user-content-direct-h[(❧)] | ``mkdir (``*__path__``,`` __mode__``);`` .4+a|
...
|       X | G link:doc/implem-notes.asciidoc#user-content-direct-h[(❧)] | ``_mkdir (``*__path__``,`` __mode__``);``
|       W |   (G)  | ``mkdir (``*__path__``);``
|       W |    =   | ``_mkdir (``*__path__``);``
...
|       W | = link:doc/implem-notes.asciidoc#user-content-dos-h[(❧)] | ``bdos (``__dos-func__``,`` __dx__``,`` __al__``);`` |
...
|       W |   :=   | ``_getpid ();`` |
|       W | = link:doc/implem-notes.asciidoc#user-content-process-h[(❧)] | ``spawnve (``__mode__``,`` *__path__``,`` *__argv__``,`` *__envp__``);`` .2+a|
			* Currently only implements the `P_WAIT` spawning mode.
			* For these functions, `libi86` purposely deviates from Open Watcom's documented behaviour in a few ways.  See the link:doc/implem-notes.asciidoc#user-content-process-h[implementation notes].
|       W | = link:doc/implem-notes.asciidoc#user-content-process-h[(❧)] | ``_spawnve (``__mode__``,`` *__path__``,`` *__argv__``,`` *__envp__``);``
4+|
...
|       W | = link:doc/implem-notes.asciidoc#user-content-libi86stdlib-h[(❧)] | *``_fullpath (``*__out-path__``,`` *__path__``,`` __size__``);`` |
...
|      W+ | = link:doc/implem-notes.asciidoc#user-content-libi86stdlib-h[(❧)] | ``_makepath (``*__path__``,`` *__drive__``,`` *__dir__``,`` *__fname__``,`` *__ext__``);`` a|
...
|       W | = link:doc/implem-notes.asciidoc#user-content-libi86stdlib-h[(❧)] | ``_splitpath (``*__path__``,`` *__drive__``,`` *__dir__``,`` *__fname__``,`` *__ext__``);`` | Long filenames, and network paths starting with two backslashes (``\\``), are not supported.
