...
|       B |    =   | ``_getdrive ();`` |
|      UB |    =   | ``getswitchar ();`` | Returns the (nominal) character for command line switches — normally ``'/'`` — per `int 0x21`, `%ax` = `0x3700`.
|       X |    =   | ``_getswitchar ();`` |
...
|      UB |    =   | ``setswitchar (``__ch__``);`` | Sets the (nominal) character for command line switches, with `int 0x21`, `%ax` = `0x3701`.
