...
|          X | = | = | = | ``_lz4len (``*__blk-src__``,`` __blk-size__``);`` | Parses an LZ4-compressed block and returns its uncompressed size, without actually unpacking it.  It is best to use this function only on trusted input.
6+|
