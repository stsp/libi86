...
|       X |    =   | ``_makefcb (``*__cmd-line__``,`` *__fcb__``,`` __opt__``);`` a|
			...
			** __result__``._status`` is either 0 (parse successful, no wildcards), 1 (parse succesfull, found wildcards), or -1 (invalid drive);
			** __result__``.tail`` points to the first unparsed character, or may be ``NULL`` if a system error occurred.
